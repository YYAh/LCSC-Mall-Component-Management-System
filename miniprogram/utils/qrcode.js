/**
 * Minimal Standalone QR Code Generator for WeChat Canvas
 */

// QR Code Type & Matrix implementation
...
/**
 * Draw QR Code to WeChat Canvas 2D
 ...
 * @param {number} size 
 */
function drawQrCodeToCanvas(ctx, text, x = 0, y = 0, size = 100) {
  ...
  for (var r = 0; r < count; r++) {
    for (var c = 0; c < count; c++) {
      if (qr.isDark(r, c)) {
        ctx.fillRect(x + Math.round(c * tileW), y + Math.round(r * tileH), Math.ceil(tileW), Math.ceil(tileH));
      }
    }
  }
}
